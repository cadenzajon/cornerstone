...
document.addEventListener('click', function(e) {
  if (e.target.classList.contains('btnflip')) {
    ...
    var back
    var input = '<p style="color:#777;"><input type="text" value="' + flipper.querySelector('h4').innerText + '"></p>'
    var textarea = '<textarea style="width:100%; max-width:32em; height:12em; color:#777;">' + flipper.querySelector('p').innerText + '</textarea><br><button class="btn">Update</button>'
    ...
    back.addEventListener('click', function(e) {
      if (e.target.classList.contains('btn')) {
        flipper.querySelector('h4').innerText = back.querySelector('input').value
